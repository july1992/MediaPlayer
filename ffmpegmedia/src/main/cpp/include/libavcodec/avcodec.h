...
/**
 * Codec supports lossy compression. MPAudio and video codecs only.
 * @note a codec may support both lossy and lossless
 * compression modes
 */
#define AV_CODEC_PROP_LOSSY         (1 << 1)
/**
 * Codec supports lossless compression. MPAudio and video codecs only.
 */
...
/**
 * MPAudio encoder supports receiving a different number of samples in each call.
 */
...
#define FF_MB_DECISION_RD     2        ///< rate distortion
    ...
    /**
     * MPAudio cutoff bandwidth (0 means "automatic")
     * - encoding: Set by user.
     * - decoding: unused
     */
    int cutoff;
...
#define FF_LEVEL_UNKNOWN -99
    ...
    /**
     * MPAudio only. The number of "priming" samples (padding) inserted by the
...
#define FF_SUB_TEXT_FMT_ASS_WITH_TIMINGS 1
    ...
    /**
     * MPAudio only. The amount of padding (in samples) appended by the encoder to
...
typedef struct AVCodecParameters {
    ...
    /**
     * MPAudio only. The channel layout bitmask. May be 0 if the channel layout is
     * unknown or unspecified, otherwise the number of bits set must be equal to
     * the channels field.
     */
    uint64_t channel_layout;
    /**
     * MPAudio only. The number of audio channels.
     */
    int      channels;
    /**
     * MPAudio only. The number of audio samples per second.
     */
    int      sample_rate;
    /**
     * MPAudio only. The number of bytes per coded audio frame, required by some
     * formats.
     *
    ...
    int      block_align;
    /**
     * MPAudio only. MPAudio frame size, if known. Required by some formats to be static.
     */
    int      frame_size;

    /**
     * MPAudio only. The amount of padding (in samples) inserted by the encoder at
    ...
    int trailing_padding;
    /**
     * MPAudio only. Number of samples to skip after a discontinuity.
     */
    int seek_preroll;
} AVCodecParameters;
...
/**
 * @defgroup lavc_resample MPAudio resampling
 * @ingroup libavc
